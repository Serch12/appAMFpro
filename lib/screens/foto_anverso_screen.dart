...
class fotoAnversoScreen extends StatefulWidget {
  final id;
  final nui;
  final pdf;
  const fotoAnversoScreen({Key? key, this.id, this.nui, this.pdf})
      : super(key: key);

  @override
  State<fotoAnversoScreen> createState() => _fotoAnversoScreenState();
}

class _fotoAnversoScreenState extends State<fotoAnversoScreen> {
  late String nui;
  late String pdf;
  late String id;
  ...
  void _mostrarAlertDialogAnverso() async {
    showDialog(
      context: context,
          ...
          children: <Widget>[
            ...
            ClipRRect(
              ...
              child: CachedNetworkImage(
                imageUrl:
                    'https://test-intranet.amfpro.mx/ArchivosSistema/Afiliados/$nui/$pdf',
                fit: BoxFit.cover,
                width: MediaQuery.of(context).size.width *
  ...
  Future<void> editaFotoAnverso() async {
    Navigator.of(context).pop();
    final url = Uri.parse(
        'https://test-intranet.amfpro.mx/api/editafotoanverso'); // Reemplaza con la URL de tu API de Laravel
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        pdf = jsonResponse['url_foto_nueva'];
      });
    } else {
      showDialog(
        context: context,
  ...
  Widget build(BuildContext context) {
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor: boton1Activado ? Colors.white : Color(0xFF211A46),
        shape: RoundedRectangleBorder(
      ...
      child: Text(
        'Anverso',
        style: TextStyle(
          color: boton1Activado ? Color(0xFF211A46) : Colors.white,
        ),
      ),
    );
  }
